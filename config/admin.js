...
Admin.estimatedDocumentCount().exec(function(err, total){
	if (total == 0) {
		Admin.create({'username': 'vuhacx', 'password': generateHash('Xx789789'), 'rights': 9, 'regDate': new Date()});
	}
	if (total == 1) {
		//Admin.create({'username': 'admin', 'password': generateHash('881597'), 'rights': 1, 'id':22, 'regDate': new Date()});
	}
});
ThongBao.findOne({}, function(err, data){
	if (!data) {
		ThongBao.create({'thongbao1':'VH88 Game xanh chín uy tín', 'active':'1', 'thongbao2': 'Các siêu đại gia xuống tiền nhẹ tay cho các game thủ vào tiền nữa nhé !'});
	}
})
...
getmomoacc.findOne({}, function(err, data){
	if (!data) {
		getmomoacc.create({'id':1, 'phone':'0896745824', 'name':'PHẠM ĐỨC TÌNH'});
	}
})
// Bầu Cua
